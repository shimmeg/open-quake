...
- **macOS status:** transport buttons and knob volume/mute go through the narrow
  `app/mediaKeys.js` adapter, whose current backend uses `robotjs` only for fixed media-key
  events. macOS may ask for Accessibility permission. Replacing that backend with a
  narrower macOS media implementation is tracked as follow-up; generic now-playing metadata is
...
*(A native macOS media-key backend, macOS now-playing backend, and album art are planned follow-ups.)*
