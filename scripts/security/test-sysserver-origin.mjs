...
try {
  // 1) The P1: protected routes with a valid loopback Host but NO Origin and NO Sec-Fetch-Site must fail closed.
  ...
  // 2) Cross-site requests are blocked.
  ...
  // 3) A foreign / DNS-rebinding Host is rejected on every route, page routes included.
  ...
  // 4) Legitimate same-origin fetches succeed.
  ...
  // 5) Page + asset routes load under a top-level navigation (Sec-Fetch-Site: none).
  ...
  console.log('sysserver origin tests passed.');
} finally {
  sysserver.stop();
}
